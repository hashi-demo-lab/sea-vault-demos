...
kubectl create secret generic my-vault-license --from-literal=license="${VAULT_LICENSE}" --namespace=my-vault-demo
...
for dc in "${(@k)datacentres}"; do
  ...
  while true; do
    ...
    for i in {0..2}; do
      pod_name="vault-${dc}-$i"
      pod_status=$(kubectl --namespace=my-vault-demo get pod "$pod_name" -o jsonpath='{.status.phase}') &> /dev/null
  ...
  init_output=$(kubectl --namespace=my-vault-demo exec "${vault_operator_pod}" -- vault operator init -key-shares=1 -key-threshold=1 -format=json)
  ...
  # Unseal leader
  kubectl --namespace=my-vault-demo exec "${vault_operator_pod}" -- vault operator unseal $(eval echo "\${${dc}_unseal_key}")
 
  # Join each cluster node to the leader
  for i in {1..2}; do
    pod_name="vault-${dc}-$i"
    echo "\n\033[32mJoining pod: ${pod_name} to vault-${dc}\033[0m\n"
    kubectl --namespace=my-vault-demo exec "${pod_name}" -- vault operator raft join "http://vault-${dc}-0.vault-${dc}-internal:8200"
  done
  ...
  for i in {1..2}; do
    ...
    kubectl --namespace=my-vault-demo exec "${pod_name}" -- vault operator unseal $(eval echo "\${${dc}_unseal_key}")
  done
